...
function animateNumber(element, finalNumber) {
    ...
    const increment = Math.ceil(finalNumber / steps);
    ...
    const timer = setInterval(() => {
        ...
        if (count >= finalNumber) {
            clearInterval(timer);
            element.textContent = finalNumber % 10;
        }
    }, stepTime);
}
...
function loadProjects(filter = currentFilter) {
    ...
    const loadMoreBtn = document.getElementById('load-more');
    if (loadMoreBtn) {
        loadMoreBtn.style.display = projectsShown >= projectsToShow.length ? 'none' : 'block';
    }
}
...
function initAboutViewMore() {
    ...
    if (viewMoreBtn && additionalContent) {
...
function setupMagneticButton() {
    ...
    if (submitBtn) {
        submitBtn.addEventListener('mousemove', (e) => {
            const position = submitBtn.getBoundingClientRect();
