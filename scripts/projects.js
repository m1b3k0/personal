...
function handleDesktopDropdownFilter() {
    const urlParams = new URLSearchParams(window.location.search);
    const filter = urlParams.get('filter');

    if (filter) {
        // Find the corresponding vertical navigation button
        const verticalNavButton = document.querySelector(`.category-btn[data-filter="${filter}"]`);
        if (verticalNavButton) {
            // Simulate a click on the vertical navigation button
            verticalNavButton.click();
        }
    }
}
...
function showProjectDetails(projectName) {
    ...
    const projectsContent = document.querySelector('.projects-content');
    const detailsContent = createProjectDetailsContent(project);

    gsap.to(projectsContent, {
        opacity: 0,
        duration: 0.3,
        onComplete: () => {
            projectsContent.innerHTML = detailsContent;
